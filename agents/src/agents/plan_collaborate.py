...
async def commit_specs(state: PlanState) -> dict[str, Any]:
    """Commit spec files to the repository."""
    repo_parts = state["repository"].split("/")
    if len(repo_parts) != 2:
...
## API Contracts
{json.dumps(state['api_contracts'], indent=2)}
...
"""
    try:
        ...
        await github_client.create_or_update_file(
            owner=owner,
            repo=repo,
            path=f".bk/specs/{feature_slug}/requirements.md",
            content=spec_content,
            message=f"docs: add spec for {feature_slug}",
            branch="main",
        )
        return {"specs_committed": True}
    except Exception as e:
